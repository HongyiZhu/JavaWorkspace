...
public class FullID {
	...
	public FullID(String _website_namespace, String _type_namespace, String _relative_ID){
		...
		namespace = (new BigInteger(website_namespace).multiply(Define.website_namespace_gain))
				.add(new BigInteger(type_namespace).multiply(Define.type_namespace_gain));
	...
	public static FullID parseAbsoluteID(BigInteger absolute_ID){
		...
		BigInteger[] temp = absolute_ID.divideAndRemainder(Define.website_namespace_gain);
		id.website_namespace = temp[0].toString();
		BigInteger[] temp1 = temp[1].divideAndRemainder(Define.type_namespace_gain);
