...
//		}
	...
	private void getTestSet(int i) {
		ResultSet rs = op.query("SELECT * FROM `new_test_set` LIMIT " + i * 200 + ", 200");
		try {
			while (rs.next()){
				ld.add(rs.getString("id"));
				md.put(rs.getString("id"), new Danmaku2(rs.getString("id"), rs.getString("content"), 
